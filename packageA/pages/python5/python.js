...
Page({
  ...
  tapSelect: function (e) {
    ...
    if (this.data.answers.activeNum + 1 < this.data.answers.allList.length) {
      ...
      if (this.data.answers.isShowTip) {
        wx.showModal({
          showCancel: false,//是否显示取消按钮
          title: '答案错误！',
          content: '正确答案：' + correct,
          success: function (res) {
            if (res.confirm) {
              console.log('错误答案')
            }
  ...
  tapConfirm: function (e) {
    ...
    wx.setStorageSync("success", pro_suc)
    wx.setStorageSync("wrong", pro_wro)
    console.log("pro_suc", pro_suc)
    console.log("pro_wro", pro_wro)
    ...
    if (this.data.answers.activeNum + 1 < this.data.answers.allList.length) {
      ...
      if (num >= 0) {
        ...
        wx.setStorageSync('suc', num)
        wx.setStorageSync('wro', this.data.answers.allList.length - num)


        wx.setStorageSync('res_answers', this.data.answers)
        console.log("this.data.answers", this.data.answers)
        //wx.setStorageSync('res_answers', questionData) 
        ...
        wx.showModal({
          ...
          success: function (res) {
            if (res.confirm) {
              wx.redirectTo({
                url: '../../../packageB/pages/showanswers/showanswers'
              })
            }
          }
        })
  ...
  onLoad: function () {
    ...
    for (var i = 0; i < questionData.data.length; i++) {
      ...
      questionData.data[i].isAnswer = 0
